...
module Header
  module ProjectsHelper
    def project_node_label(project, favorited: false)
      ...
      if favorited
        parts << render(Primer::Beta::Octicon.new(icon: :"star-fill", size: :small, classes: "op-primer--star-icon", ml: 2))
      end

      if show_workspace_type_badge?(project)
        parts << render(Primer::BaseComponent.new(tag: :div, display: :inline_flex, align_items: :center,
                                                  color: :subtle, font_size: :small, ml: 2)) do
          safe_join([
      ...
      parts.length == 1 ? parts.first : safe_join(parts)
    end
