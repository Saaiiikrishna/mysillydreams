...
RSpec.describe WorkPackages::Scopes::WithBacklogsNeighbours do
  ...
  describe ".with_backlogs_neighbours" do
    ...
    context "when the scope excludes closed work packages" do
      ...
      subject { neighbours(wp3, scope: WorkPackage.where(project:, status: open_status).order_by_position) }

      it { is_expected.to have_attributes(prev_prev_id: nil, prev_id: wp1.id, next_id: nil) }
    end
    ...
    context "when the scope excludes work packages of an excluded type" do
      ...
      let!(:wp1) { create(:work_package, project:, status: open_status, type: included_type, position: 1) }
      let!(:excluded_wp) { create(:work_package, project:, status: open_status, type: excluded_type, position: 2) }
      let!(:wp3) { create(:work_package, project:, status: open_status, type: included_type, position: 3) }

      subject { neighbours(wp3, scope: WorkPackage.where(project:, type: included_type).order_by_position) }
