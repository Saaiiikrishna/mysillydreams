  ...
  describe "move menu items" do
    ...
    context "when item is first" do
      ...
      it "shows Move down and Move to bottom, sending next_id as prev_id" do
        ...
        expect(page).to have_text(I18n.t(:label_sort_lower))
        expect(page).to have_text(I18n.t(:label_sort_lowest))
        expect(page).to have_css("input[name='prev_id'][value='#{second_story.id}']", visible: :hidden)
      end
    end

    context "when item is in the middle with one predecessor" do
      it "shows all move options, sending nil prev_id for Move up and next_id for Move down" do
        render_component
        ...
        expect(page).to have_text(I18n.t(:label_sort_lower))
        expect(page).to have_text(I18n.t(:label_sort_lowest))
        expect(page).to have_css("input[name='prev_id'][value='']", visible: :hidden)
        expect(page).to have_css("input[name='prev_id'][value='#{third_story.id}']", visible: :hidden)
      end
    end
    ...
    context "when item is in the middle with two predecessors" do
      ...
      it "shows all move options, sending prev_prev_id and next_id as prev_id" do
        ...
        expect(page).to have_text(I18n.t(:label_sort_lower))
        expect(page).to have_text(I18n.t(:label_sort_lowest))
        expect(page).to have_css("input[name='prev_id'][value='#{first_story.id}']", visible: :hidden)
        expect(page).to have_css("input[name='prev_id'][value='#{fourth_story.id}']", visible: :hidden)
      end
    end
